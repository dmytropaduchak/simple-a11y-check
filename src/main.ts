...
import { scanDiffForA11y, type Finding } from "./rules";

const MARKER = "<!-- simple-a11y-check -->";
const NAME = "Simple A11y Check";

function formatFindings(findings: Finding[]): string {
  if (!findings.length) {
    return [MARKER, `## ${NAME}`, "", "No common a11y anti-patterns found in PR-added UI lines."].join("\n");
  }
  const rows = findings
...
async function run(): Promise<void> {
  ...
  const findings = scanDiffForA11y(diff);
  const summary = formatFindings(findings);
  await core.summary.addRaw(summary, true).write();
  for (const f of findings) {
    if (f.severity === "high") core.error(`${f.title} (${f.ruleId})`, { file: f.file, startLine: f.line });
    else core.warning(`${f.title} (${f.ruleId})`, { file: f.file, startLine: f.line });
  }
  try {
  ...
  const shouldFail =
    failOn === "high"
      ...
      : failOn === "medium"
        ...
        : false;
  if (shouldFail) core.setFailed(`simple-a11y-check: ${findings.length} finding(s)`);
